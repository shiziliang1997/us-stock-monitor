...
def send_wechat_push(title, content):
    ...
    """
    if not PUSHPLUS_TOKEN or PUSHPLUS_TOKEN == "这里填你的新Token":
...
def clean_yfinance_df(df, ticker):
    """
    修复 yfinance 在 Colab / Jupyter 中可能出现的多层列问题
    """
...
def main():
    ...
    report_content = "## 美股监控日报\n\n"

    report_content += "### 一、市场环境\n\n"
    ...
    report_content += "\n---\n\n"
    report_content += "### 二、个股监控\n\n"

    for _, row in result_df.iterrows():
        report_content += f"""
### {row['股票']} {row['名称']}｜{row['信号灯']}
